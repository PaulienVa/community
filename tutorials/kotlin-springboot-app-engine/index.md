...
1.  
    ...
    By specifying `runtime: java`, the runtime image `gcr.io/google-appengine/openjdk:8` is automatically selected
    when you deploy a JAR (*.jar) file. The JDK version is also selected using the `jdk` field.
    
    _When running on JDK 11:_
    Create a file called `app.yaml` in the root directory with the following contents:
